...
export function buildPreLiveDryRun(project: Project, store: MeridianStore): PreLiveDryRunResult {
  ...
  if (!entries.every((entry) => numberOk(entry.stopLossPct, -99, -0.000001) && entry.takeProfitPercents.length > 0 && numberOk(entry.perTxSellCapPct, 0.000001, 100))) blockers.push('risk-rules-missing');
  ...
  if (process.env.LIVE_TRADING_ENABLED === 'true') blockers.push('live-trading-enabled');
