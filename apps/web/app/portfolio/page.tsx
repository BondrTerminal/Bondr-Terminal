        ...
        {view === 'wallets' ? (
          ...
          <>
            ...
            <section className="portfolioPositionsPanel">
              ...
              {tableTab === 'history' ? <div className="portfolioTable historyTable" role="table"><div className="portfolioRow portfolioHead"><span>Type</span><span>Token</span><span>Amount</span><span>Market Cap</span><span>Age</span><span>Explorer</span></div>{history.length ? history.map((event) => <div className="portfolioRow" key={String(event.id)}><strong>{text(event.type)}</strong><span>{text(event.projectId)}</span><span>{numberText(event.tokenAmount)} · {numberText(event.solAmount)} SOL</span><span>—</span><span>{String(event.timestamp ?? '').slice(0, 10) || '—'}</span><span>—</span></div>) : <div className="emptyPortfolioState">No activity</div>}</div> : tableTab === 'top100' ? <div className="portfolioTable" role="table"><div className="portfolioRow portfolioHead"><span>Token</span><span>Bought</span><span>Sold</span><span>Remaining</span><span>PNL</span><span>Action</span></div>{top100.length ? top100.map((row) => <div className="portfolioRow" key={String(row.projectId)}><strong>{text(row.symbol, text(row.name, 'Token'))}</strong><span>{numberText(row.bought)}</span><span>{numberText(row.sold)}</span><span>{numberText(row.remaining)}</span><span className={Number(row.totalPnlUsd ?? 0) >= 0 ? 'profitText' : 'dangerText'}>{money(row.totalPnlUsd, text(row.pnlStatus, 'unavailable'))}</span><span>{row.mint ? <a href={`/sniper?mint=${row.mint}&project=${row.projectId}`}>Open Terminal</a> : 'No mint'}</span></div>) : <div className="emptyPortfolioState">No top trades</div>}</div> : <div className="portfolioTable" role="table"><div className="portfolioRow portfolioHead"><span>Token</span><span>Bought</span><span>Sold</span><span>Remaining</span><span>PNL</span><span>Action</span></div>{activePositions.length ? activePositions.map((row) => <div className="portfolioRow" key={String(row.projectId)}><strong>{text(row.name, 'Token')} <em>{text(row.symbol)}</em></strong><span>{numberText(row.bought)}</span><span>{numberText(row.sold)}</span><span>{numberText(row.remaining)}</span><span className={Number(row.totalPnlUsd ?? 0) >= 0 ? 'profitText' : 'dangerText'}>{money(row.totalPnlUsd, text(row.pnlStatus, 'unavailable'))}</span><span>{row.mint ? <a href={`/sniper?mint=${row.mint}&project=${row.projectId}`}>Open Terminal</a> : 'No mint'}</span></div>) : <div className="emptyPortfolioState">No active positions</div>}</div>}
            </section>
