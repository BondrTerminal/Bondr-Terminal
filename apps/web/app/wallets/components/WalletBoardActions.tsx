...
export function WalletBoardActions({ wallets, groups, selectedProjectName, selectedProjectId, selectedGroupId, totalSol, activeCount, archivedCount, hydrationStatus, hydrationProvider }: Props) {
  ...
  async function connectPhantom() {
    ...
    try {
      ...
      setMessage({ type: 'ok', text: address ? `Phantom connected: ${address.slice(0, 4)}…${address.slice(-4)}. No transaction signature requested.` : 'Phantom opened. No transaction signature requested.' });
    } catch (error) {
    ...
    <section className="walletBoardShell">
      <header className="walletBoardHeader">
        ...
        <div className="walletBoardActions">
          ...
          <ActionButton onClick={() => openAction('create')}>Create Local Wallet</ActionButton>
          <ActionButton onClick={() => openAction('import')}>Import Local Wallet</ActionButton>
          ...
          {visibleWallets.map((wallet) => (
            <div className="walletListRow" role="row" key={wallet.id} onDoubleClick={() => setDetailWalletId(wallet.id)}>
              ...
              <div className="walletRowActions">
                ...
                <button type="button" onClick={() => { setFromWalletId(wallet.id); openAction('export'); }}>{wallet.custodyMode === 'managed-local' ? 'Backup / Export' : 'Public Record'}</button>
