...
function actionTitle(action: WalletAction) {
  ...
  if (action === 'export') return 'Backup / export';
  return '';
}
...
export function WalletBoardActions({ wallets, groups, selectedProjectName, selectedProjectId, selectedGroupId, totalSol, activeCount, archivedCount, hydrationStatus, hydrationProvider }: Props) {
  ...
  function openAction(nextAction: Exclude<WalletAction, null>) {
    if (!MANAGED_LOCAL_WALLET_BETA_ENABLED && (nextAction === 'create' || nextAction === 'import')) {
      setMessage({ type: 'warn', text: 'Managed local wallet create/import is disabled. Use Connect Phantom or Track Address/watch-only instead.' });
      setAction(null);
      return;
    }
    setExportedSecret('');
  ...
  async function previewImportPrivateKey() {
    if (!MANAGED_LOCAL_WALLET_BETA_ENABLED) {
      setMessage({ type: 'warn', text: 'Private-key import is disabled.' });
      return null;
    }
  ...
  async function createManagedWallet(importing: boolean) {
    if (!MANAGED_LOCAL_WALLET_BETA_ENABLED) {
      setMessage({ type: 'warn', text: 'Managed local wallets are disabled. Use Phantom/Solflare plus watch-only public records.' });
      return null;
    }
  ...
  async function exportPrivateKey() {
    if (!MANAGED_LOCAL_WALLET_BETA_ENABLED) {
      setMessage({ type: 'warn', text: 'Private-key export is disabled.' });
      return null;
    }
    ...
    <section className="walletBoardShell">
      <header className="walletBoardHeader">
        ...
        <div className="walletBoardActions">
          ...
          <ActionButton onClick={() => openAction('create')} disabled={!MANAGED_LOCAL_WALLET_BETA_ENABLED} title="Disabled">Create Local Wallet</ActionButton>
          <ActionButton onClick={() => openAction('import')} disabled={!MANAGED_LOCAL_WALLET_BETA_ENABLED} title="Disabled">Import Local Wallet</ActionButton>
          ...
          <ActionButton onClick={() => openAction('export')}>Backup / Export</ActionButton>
          ...
          {visibleWallets.map((wallet) => (
            <div className="walletListRow" role="row" key={wallet.id} onDoubleClick={() => setDetailWalletId(wallet.id)}>
              ...
              <div className="walletRowActions">
                ...
                <button type="button" onClick={() => { setFromWalletId(wallet.id); openAction('export'); }}>{wallet.custodyMode === 'managed-local' && MANAGED_LOCAL_WALLET_BETA_ENABLED ? 'Backup / Export' : 'Public Record'}</button>
                ...
                <a href={deploymentHref}>Deploy</a><a href={terminalHref}>Terminal</a><a href={portfolioHref}>Portfolio</a>
              </div>
            </div>
          ))}
      ...
      {action && (
        <div className="walletModalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setAction(null); }}>
          <section className="walletActionModal" role="dialog" aria-modal="true" aria-label={actionTitle(action)}>
            ...
            {['create', 'import'].includes(action) && !MANAGED_LOCAL_WALLET_BETA_ENABLED && <div className="walletScaffoldPanel"><strong>Managed local wallets disabled</strong><p>Bond.Terminal currently supports Phantom/Solflare custody and watch-only public address records. No private-key import, generation, reveal, or vault custody is available.</p><button className="walletModalPrimary" type="button" onClick={() => setAction('phantom')}>Connect Phantom instead</button><button className="walletModalPrimary" type="button" onClick={() => setAction('track')}>Track watch-only address</button></div>}
