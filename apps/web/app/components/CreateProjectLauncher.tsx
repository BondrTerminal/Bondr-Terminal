...
export function CreateProjectLauncher({
  ...
  async function createProject() {
    ...
    try {
      ...
      setTimeout(() => { router.push(projectId ? `/deployment?project=${projectId}` : '/deployment'); }, 500);
    } catch (caught) {
  ...
  const launcherCard = (
    <section className={`documentCard createProjectPanel guidedCreateProjectPanel ${mode === 'inline' ? 'inlineCreateProjectPanel' : ''} ${className}`}>
      <div className="sectionIntro compactIntro createProjectHeaderRow">
        ...
        <div className="createProjectHeaderActions"><button className="button" type="button" onClick={() => void openLauncher()}>{label}</button>{mode === 'inline' && <><Link href="/projects">Projects</Link><Link href="/deployment">Deployment</Link></>}</div>
      </div>
  ...
  function wizard(isModal: boolean) {
    const shell = <div className="createProjectModalShell professionalCreateProjectShell">
      ...
      <section className="createProjectStepBody professionalCreateProjectBody">
        ...
        {step === 'wallet' && <div className="overviewDeployPanel launchReviewPanel"><h3>Wallet routing</h3><div className="walletBoxLayout createProjectWalletBox"><div className="walletBoxLayer walletBoxTopLayer"><span>{rail?.selectedInventoryMatch ? 'Saved in Portfolio' : 'Needs record'}</span><em>{rail?.provider ?? 'checking'}</em></div><div className="walletBoxLayer walletBoxIdentityLayer"><strong>{activeWallet ? activeWallet.role : 'No active wallet selected'}</strong><code>{activeWallet ? short(activeWallet.address) : 'connect or select wallet'}</code></div><div className="walletBoxLayer walletBoxMetaLayer"><span>{typeof rail?.selectedSolBalance === 'number' ? `${rail.selectedSolBalance.toFixed(5)} SOL` : rail?.balanceStatus ?? 'checking'}</span><small>Browser signer must match selected wallet later</small></div></div><label><span>Project wallet group</span><select value={walletGroupId} onChange={(event) => setWalletGroupId(event.target.value)}>{groups.map((group) => <option value={group.id} key={group.id}>{group.name} · {group.scope} · {group.walletIds?.length ?? 0} wallets</option>)}{!groups.length && <option value="">No groups loaded</option>}</select></label><p className="qaMuted">This links a public Portfolio wallet record to the project. Your browser wallet signs later only when the connected signer matches the selected wallet. No key material is stored.</p>{!activeWallet && <Link className="button secondary" href="/portfolio?view=wallets">Open Portfolio Wallets</Link>}</div>}
