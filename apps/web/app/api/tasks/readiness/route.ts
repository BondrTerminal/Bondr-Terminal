...
import { buildTaskExecutionReadiness } from '../../../../lib/sniper-task-readiness';
...
export async function GET(request: Request) {
  ...
  const readiness = buildTaskExecutionReadiness(active?.project ?? null, active?.wallets ?? store.wallets.filter((wallet) => !wallet.archived), getLiveActivationStatus());
