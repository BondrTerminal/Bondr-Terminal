...
export function ExecutionDock({ mint, selectedWalletLabel, wallets = [] }: { mint?: string; selectedWalletLabel: string; wallets?: DockWallet[] }) {
  ...
  const blockReasons = [
    ...
    capabilities?.broadcastEnabled === false ? 'broadcast disabled by policy' : null
  ...
  async function signInWallet() {
    ...
    try {
      ...
      setLiveMessage(capabilities?.broadcastEnabled ? 'Signed locally and review passed. Broadcast is available only through the separate submit step.' : 'Signed locally and review passed. Broadcast is disabled by policy.');
    } catch (error) { setLiveMessage(error instanceof Error ? error.message : 'Wallet signing failed or was rejected.'); }
    finally { setLiveLoading(false); }
  }

  async function submitBroadcast() {
    if (!capabilities?.broadcastEnabled) { setLiveMessage('Broadcast is disabled by policy.'); return; }
  ...
  return (
    <aside className="executionDock premiumExecutionDock">
      <section className="dockCard orderTicketCard premiumOrderCard">
        <div className="dockHeader tradingPanelHeader proPaperHeader"><span>Terminal</span><strong>Quote → build → simulate → sign</strong><small>Broadcast policy-gated</small></div>
